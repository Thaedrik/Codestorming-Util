 ...
 */
package org.codestorming.util.pool;
...
public class ObjectPool<T> {
	...
	/**
	 * Creates a new {@code ObjectPool} with the given {@link org.codestorming.util.pool.ObjectPool.ObjectFactory} and
	 * maximum pool size.
	 * <p/>
	...
	public ObjectPool(ObjectFactory<T> objectFactory, int maxPoolSize) {
		...
		reclaimed = new HashSet<SoftReference<T>>(maxPoolSize);
	...
	public void putback(T object) {
		if (object != null &&
			...
			synchronized (reclaimed) {
				checkReferences();
				reclaimed.add(new SoftReference<T>(object, queue));
			}
		}
	}
